...
const JobHunting = (props) => {
  ...
  const saveJob = () => {
    ...
    const jobApps = JSON.parse(localStorage.getItem('freelance-job-apps')) || {};
    ...
    localStorage.setItem('freelance-job-apps', JSON.stringify(jobApps));
  ...
  const modifyJob = () => {
    ...
    localStorage.setItem('freelance-job-apps', JSON.stringify(updatedJobApps));
  ...
  return <div className="JobHunting">
    ...
    <div className="JobHunting__body">
      ...
      {!editing && !showAddJobAppModal && <div className="JobHunting__freelance-job-apps">{renderJobApps()}</div>}
    </div>
  </div>
}
